...
with urllib.request.urlopen(req1) as resp1:
    raw_token = json.loads(resp1.read().decode("utf-8")).get("h", "")

print("RAW TOKEN FROM play.php:", raw_token)

# If raw_token starts with 'in=', don't add 'in=' again
if raw_token.startswith("in="):
    token_param = raw_token
else:
    token_param = f"in={raw_token}"

# Step 2: Fetch Master M3U8 from net77.cc
master_url = f"{mainUrl}/hls/70021642.m3u8?{token_param}"
    ...
    for domain in candidate_domains:
        ...
        try:
            ...
            with urllib.request.urlopen(r, timeout=5) as res:
                ...
                print(f"    SUCCESS! [{res.status}] Length: {len(content)}")
